...
public class Apanel extends JPanel {
    private String num;
	public Apanel(String num) {// 第一位抽号号码的走势面板
        this.num = num;
        ...
        // 去掉固定的 a
        int i0 = con.getABC(num, 0);// 获得第一位抽号号码0出现的次数
        int i1 = con.getABC(num, 1);// 获得第一位抽号号码1出现的次数
        int i2 = con.getABC(num, 2);// 获得第一位抽号号码2出现的次数
        int i3 = con.getABC(num, 3);// 获得第一位抽号号码3出现的次数
        int i4 = con.getABC(num, 4);// 获得第一位抽号号码4出现的次数
        int i5 = con.getABC(num, 5);// 获得第一位抽号号码5出现的次数
        int i6 = con.getABC(num, 6);// 获得第一位抽号号码6出现的次数
        int i7 = con.getABC(num, 7);// 获得第一位抽号号码7出现的次数
        int i8 = con.getABC(num, 8);// 获得第一位抽号号码8出现的次数
        int i9 = con.getABC(num, 9);// 获得第一位抽号号码9出现的次数
        con.closeConnection();
        double all = i0 + i1 + i2 + i3 + i4 + i5 + i6 + i7 + i8 + i9;// 获得抽号次数
        DecimalFormat df = new DecimalFormat(".###");// 格式化（保留三位有效数字）
        // 设置统计抽号号码出现次数标签中的文本内容
        l_0.setText("出现 " + i0 + " 次");
        l_1.setText("出现 " + i1 + " 次");
        l_2.setText("出现 " + i2 + " 次");
        l_3.setText("出现 " + i3 + " 次");
